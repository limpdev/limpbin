...
func releaseCOM(obj IUnknown) {
	if obj != 0 {
		syscall.Syscall(obj+2*unsafe.Sizeof(uintptr(0)), 1, uintptr(obj), 0, 0) // Call Release method (IUnknown index 2)
	}
}
...
func wndProc(hwnd windows.HWND, msg uint32, wParam, lParam uintptr) uintptr {
	...
	case WM_PAINT:
		...
		var ps struct { // PAINTSTRUCT - needed for BeginPaint/EndPaint
			hdc         windows.Handle
			fErase      int32
			rcPaint     RECT
			fRestore    int32
		...
		// BeginPaint(hwnd, &ps) // Or ValidateRect if no painting needed here
		// EndPaint(hwnd, &ps)
		syscall.Syscall(user32.NewProc("ValidateRect").Addr(), 2, uintptr(hwnd), 0, 0) // Avoid default painting
		return 0
	default:
