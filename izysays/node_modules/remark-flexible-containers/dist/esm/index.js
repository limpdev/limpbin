...
export const plugin = (options) => {
    ...
    const constructTitle = (type, title, props) => {
        ...
        const titleTagName = typeof settings.titleTagName === "string"
            ? settings.titleTagName
            : settings.titleTagName(_type, _title);
        const titleClassName = typeof settings.titleClassName === "string"
    ...
    const constructContainer = (children, type, title, props) => {
        const _type = type?.toLowerCase();
        const _title = title?.replace(/\s+/g, " ");
        ...
        const containerTagName = typeof settings.containerTagName === "string"
            ? settings.containerTagName
            : settings.containerTagName(_type, _title);
        const containerClassName = typeof settings.containerClassName === "string"
     ...
     *
     */
    function getSpecificIdentifiers(input) {
        if (!input)
            return { containerProps: undefined, title: undefined, titleProps: undefined };
        ...
        const containerProps = containerFixture && containerFixture !== "" ? containerFixture?.split(" ") : undefined;
    ...
    function analyzeChild(node) {
        const textElement = node.children[0]; // it is guarenteed in "checkTarget"
        let flag = undefined;
        let type = undefined;
        let title = undefined;
        let nIndex = undefined; // for newline "\n" character
    ...
    function analyzeChildren(node) {
        const firstElement = node.children[0]; // it is guarenteed in "checkTarget"
        let flag = "mutated"; // it has more children means it can not be "regular"
        let type = undefined;
        let title = undefined;
     ...
     *
     */
    function checkParagraphWithEmptyText(node) {
     ...
     *
     */
    function deleteFirstChildBreak(node) {
    ...
    const transformer = (tree) => {
        ...
        visit(tree, "paragraph", function (node, index, parent) {
            ...
            if (!isTarget)
                return;
            const { flag, type, rawtitle } = node.children.length === 1
